...
import com.example.rksishedule.data.RVhistoryAdapter.DayViewHolder


class RVadapter (
    val context: Context,
    private var recyclerData: Shedule
) :
...
//        kotlin.io.println("$viewType\nholder - $holder")
        if (viewType == DAY_VIEW_TYPE) {
            ...
            if (uView == "easy") {
                ...
                println(""+dayViewHolder.day.text.length+" "+"LENG")
            }
        ...
        if (viewType == SHEDULE_VIEW_TYPE) {
            ...
            if (item.getLessons()[positionLesson] == inviseLessonName) {
...
//        }
    ...
    inner class DayViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView), OnClickListener {
        ...
        init {
            card.setOnClickListener { view ->
                functionsApp.showPopupTimeNotify(context, recyclerData, position-1)
            }
        }
    ...
    inner class SheduleViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView), OnClickListener {
        ...
        init {
            card.setOnClickListener {
                functionsApp.showPopupItem(context, recyclerData, position-1)
            }
        }
